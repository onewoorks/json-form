...
<div class='form-group form-group-sm'>
    <p style='padding-left:15px'><b>Are you sure want to delete <strong class="text-uppercase"><?= $predefine->multiple_desc;?></strong> ?</b></p>
...
<script>
    $('.yesDelete').click(function () {
        var values = $('#deleteDocument').serializeArray();
            $.ajax({
                url: '<?= SITE_ROOT; ?>/formview/delete-current-predefine/',
                data: {values:values},
                success: function (data) {
//                    console.log(data);
                    $('#deleteModal').modal('hide');
                    swal({
                        title:"Multiple Answer Removed!",
